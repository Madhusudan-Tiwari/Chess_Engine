...
void move_piece(char board[8][8], Position from, Position to, char promotion_piece)
{
    ...
    if (piece == 'P' && to.row == 0)
    {
        ...
        {
            board[to.row][to.col] = toupper(promotion_piece);
        }
        else
        {
            while (true)
            {
                cout << "Please enter the piece you want "
                        "to promote it to (Q, B, N, R): ";
...
{
    update_castling_flags(state, move.from, move.to, move.piece);

    move_piece(state.board, move.from, move.to, move.promotion);
